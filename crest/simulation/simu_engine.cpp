...
void CSimuEngine::preStartSimulation(void)
{
	for (int i=0; i<m_nObjects; i++){
		...
		pobj->setTimeStepForMaterialRelatedIssues(m_timestep);
	}
}

//advance one time step
inline void CSimuEngine::_advanceOneTimeStep(const unsigned int stepID)
{
	for (int i=0; i<m_nObjects; i++){
		CSimuEntity* pobj = m_pObjects[i];
...
void CSimuEngine::startSimulation(const unsigned int nsteps, const unsigned int outputstep)
{
	char objfilename[200];
	unsigned int N = outputstep;
	...
	for (unsigned int i=0; i<nsteps; i++){
		unsigned int &m_nStepID = i;
		if ((m_nStepID%N==0) && (m_nStepID>=m_nMinOutputStep)){
	...
	const unsigned int nsteps, const unsigned int outputstep, const char *colfilename)
{
	...
	const unsigned int N = outputstep;
	const double TIMEOUT=1.0/30;
	Vector3d colpos[100];
	int vertexid[100];
	int ncollision;

	for (unsigned int i=0; i<nsteps; i++, phytime+=m_timestep){
		if (phytime>=TIMEOUT){
			phytime=0;
			if (!col.eof()){
	...
	const double &cur_time, const double& inputTimeInterval)
{
	...
	Vector2i vrange[BUFFLENGTH];
	vrange[0].x = 0; vrange[0].y = m_pObjects[0]->getVertexCount();
	for (i=1; i<m_nObjects; i++){
	...
	for (i=0; i<m_nObjects; i++){
		...
		if (cc>BUFFLENGTH){
			...
			m_pObjects[i]->updateCollisionConstraint(
				vposbuff, vidbuff, cc, cur_time, inputTimeInterval);
		}
	}
}
