	...
	Vector3d &facenorm0, Vector3d &facenorm1, double3x3& mat, double &xlen)
{
	...
	Vector3d &Y = *((Vector3d*)(&mat.x[3]));
	Vector3d &Z = *((Vector3d*)(&mat.x[6]));
	const Vector3d p23 = (p1+p2+p3)*0.25;	//quad center
	const Vector3d p45 = (p1+p4+p5)*0.25;	//quad center
	X = p1; 
	xlen = Magnitude(X);
	X /= xlen;	//normalize
	Y0 = p23;
...
const double HEIGHT_DELTA = (2.0/1024);
	...
	const double& thickness)
{
	...
	computeQuadDeriv(qq1, qq2, qq3, qq2u, qq3u, derivA);
	computeQuadDeriv(qq1, qq4, qq5, qq4u, qq5u, derivB);
	...
	//volume sign
	double vol = TET_VOLUME(q1, q2, q3);
	m_volsign0 = 0;
	if (vol>0) m_volsign0=1;
	else if (vol<0) m_volsign0=-1;
	vol = TET_VOLUME(q1, q4, q5);
	m_volsign1 = 0;
	if (vol>0) m_volsign1=1;
	else if (vol<0) m_volsign1=-1;
}
	...
	double3x3 *ppJacobian[7])		//jocobian matrices, if needed (not NULL)
{
	...
	//spring force
	const double E = mtl.getYoung()*m_thickness;
	const double K= -10*E*(m_area0+m_area1)/(m_len0*m_len0);
