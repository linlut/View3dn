...
{
	return _exportMeshPLT(fp);
}

static void CreateEdgesForTriMesh(
	const Vector3d *pVertex, const Vector3i *pelm, const int nelm,  
	const double &thickness, const double rho, CEdgeMap& edgemap)
{
	for (int i=0; i<nelm; i++){
		...
		const double area = triangle_area(pVertex[tri.x], pVertex[tri.y], pVertex[tri.z]);
		const double mass = area *thickness * rho *0.33333333;
		for (int j=0; j<3; j++){
...
static void CreateEdgesForQuadMesh(
	const int rigidlevel, const Vector3d *pVertex, 
	const Vector4i *pelm, const int nelm, 
	const double &thickness, const double& rho, CEdgeMap& edgemap)
{
	if (rigidlevel<1) return;
	for (int i=0; i<nelm; i++){
		const Vector4i quad = pelm[i];
		const double area = quad_area(pVertex[quad.x], pVertex[quad.y], pVertex[quad.z], pVertex[quad.w]);
		const double mass = area * thickness * rho * 0.25;
		Vector2i edges[4]={
	...
	for (int i=0; i<nelm; i++){
		const Vector4i quad = pelm[i];
		const double area = quad_area(pVertex[quad.x], pVertex[quad.y], pVertex[quad.z], pVertex[quad.w]);
		const double mass = area * thickness * rho / 6;
...
void buildEdgeMapFromSurfaceElements(
	const int springtype,	//0: van Gedar 94?, 1: Lloyd 2008 TVCG
	const Vector3d *pVertex, const int elmtype, const int *pElement, const int nelm, const double &thickness, const double& rho, 
	CEdgeMap& edgemap)
{
	assert(elmtype==4 || elmtype==3);
	if (elmtype==3){//triangle mesh
		const Vector3i *pelm = (const Vector3i *)pElement;
		switch (springtype){
			case 0:	//Van Gader's method
			case 1: //Lloyd's method TVCG2007
			default:
				//CreateEdgesForTetsLloyd(pVertex, pElement, nelm, rho, edgemap);
				CreateEdgesForTriMesh(pVertex, pelm, nelm, thickness, rho, edgemap);
				break;
		};
		return;
	}
	...
	//elmtype==4 Quad mesh
	//CreateEdgesForHexahedra(pVertex, pElement, nelm, rho, edgemap);
	const Vector4i *pelm = (const Vector4i *)pElement;
	const int rigidlevel = 1;
	CreateEdgesForQuadMesh(rigidlevel, pVertex, pelm, nelm, thickness, rho, edgemap);
}
...
void CSimuShell::buildRelaxationEdges(const Vector3d *pVertex, const int rigidlevel)
{
	...
	if (m_nSurfaceType==3){
		const Vector3i *ptri = (const Vector3i*)m_pSurfacePoly;
		CreateEdgesForTriMesh(pVertex, ptri, m_nSurfacePoly, thickness, rho, edgemap);
	}
	else{
		const Vector4i* pQuad = (const Vector4i*)m_pSurfacePoly;
		CreateEdgesForQuadMesh(rigidlevel, pVertex, pQuad, m_nSurfacePoly, thickness, rho, edgemap);
	}
