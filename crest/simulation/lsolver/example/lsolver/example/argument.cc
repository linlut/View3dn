...
ArgumentsData::
ArgumentsData( void ) {
  for ( vector<string*>::iterator pos=aux.begin(); pos!=aux.end(); ++pos )
    delete[] *pos;
}
...
get_long_argument( const char *argname, long default_value ) const {
  vector<string> &argv((vector<string>&)data->argv);
...
get_rest( int &n, string *&rest ) const {
  const vector<string> &argv(data->argv);
  vector<string*>      &aux((vector<string*>&)data->aux);
#ifndef __KCC
  n=0;
...
#endif
  rest=new string[n];
  ((vector<string*>&)aux).push_back(rest);          // kein mutable vorhanden
  int k=0;
  for ( vector<string>::const_iterator pos=argv.begin();
	pos!=argv.end();
	++pos )
    if ( *pos!=0 )
      rest[k++]=*pos;
...
no_more_args( void ) const {
  const vector<string> &argv(data->argv);
  vector<string>::const_iterator first_nonzero = 
