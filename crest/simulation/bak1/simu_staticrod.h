...
private:
	...
	bool _exportMeshPLT(FILE *fp)
	{
		...
		const int npoly = m_nElementCount;
		...
		//compute TR3
		for (i=0; i<nv; i++) m_pVertInfo[i].m_var = 0;
		for (i=0; i<npoly; i++){
			...
			const int x = ll.x;
			const int y = ll.y;
			if (fabs(m_pVertInfo[x].m_var)<I1) m_pVertInfo[x].m_var=I1;
			if (fabs(m_pVertInfo[y].m_var)<I1) m_pVertInfo[y].m_var=I1;			
		}
...
public:
	 ...
	 :CSimuEntity(pVertex, nv, init_velocity, &mtl)
	{
		...
		_computeNodeMass(pVertex, (const Vector2i*)pElement, nelm, rod_crossarea, mtl.m_rho);
