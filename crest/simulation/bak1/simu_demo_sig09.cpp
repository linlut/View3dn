...
void test_mainCLOTHDRAPE(void)  //CL_DRAPE_SPH CLOTH drape on sphere
{
	...
	CSimuEngine e;
	e.addSimuObject(s);
	e.setTimestep(5.00E-3/K); // max step for verlet

	const int timerid = 0;
	startFastTimer(timerid);
	e.startSimulationInMiliSeconds(80000, 400);
...
void test_mainVBEAM(void)  //V shaped Beam
{
	//typedef CSimuSpringThinshell T;
	...
	mtl.setPoisson(0.30);
	mtl.setDamping(D, 0.100);
	mtl.m_rigid = 1;	//set the shell to be a rigid shell using relaxation
	mtl.setMassDamping(1.2500E-4);	

	CSimuEngine e;
	const double tmstep = 1.00e-3;
	e.setTimestep(tmstep/K); // max step for verlet
	...
	const int timerid = 0;
	startFastTimer(timerid);
	const double simutimeMS = 1000;  //in ms
	e.startSimulationInMiliSeconds(simutimeMS, 500);
	//e.startSimulation(40000, 200);
	stopFastTimer(timerid);
	const double t = reportTimeDifference(timerid, "Simmu. run time:");
	printf("Time step length %lg ms.\n", tmstep*1000);
	printf("Physical time is %lg ms, efficiency %lg\n", simutimeMS, simutimeMS/t);
}
...
void test_mainBUNNY(void)  //BUNNY
{
	typedef CSimuThinshell T;
	CMeMaterialProperty mtl;
	...
	mtl.setPoisson(0.30);
	mtl.setDamping(40.000, 0.100);
	mtl.m_rigid = 0;	//set the shell to be a rigid shell using relaxation
	mtl.setMassDamping(2.0e-4);	

	CSimuEngine e;
	const double tmstep = 3.00E-4;
	e.setTimestep(tmstep); // max step for verlet
	...
	e.startSimulation(4000, 20);
	stopFastTimer(timerid);
	const double t = reportTimeDifference(timerid, "Bunny simmulation time:");
}

void test_mainHEAD(void)  //head
{
