...
void CThinshell2Element::_initShearElements(
		const int isStatic, const Vector3d p[], const Vector3d wpnorm[], const double trussvolumes[])
{
	...
	double3x3 rot; rot.IdentityMatrix();
	if (!isStatic){
	...
	for (int i=0; i<m_nRod; i++){
		...
		m_shearelm[i].init(pj, p0, rot, trussvol);
	}
}
