...
void computeStiffnessMat(const Vector3d deriv[5], const double& K, double3x3 *ppJacobian[10])
{
	...
	const double af = DotProd(deriv[0], deriv[5]);
	...
	const double bf = DotProd(deriv[1], deriv[5]);
	const double cc = DotProd(deriv[2], deriv[2]);
	const double cd = DotProd(deriv[2], deriv[3]);
	const double ce = DotProd(deriv[2], deriv[4]);
	const double cf = DotProd(deriv[2], deriv[5]);
	const double dd = DotProd(deriv[3], deriv[3]);
	const double de = DotProd(deriv[3], deriv[4]);
	const double df = DotProd(deriv[3], deriv[5]);
	const double ee = DotProd(deriv[4], deriv[4]);
	const double ef = DotProd(deriv[4], deriv[5]);
	const double ff = DotProd(deriv[5], deriv[5]);
	const double S11 = aa*K;
	const double S12 = ab*K;
	const double S13 = (ac+ae)*K;
	const double S14 = (ad+af)*K;
	const double S22 = bb*K;
	const double S23 = (bc+be)*K;
	const double S24 = (bd+bf)*K;
	const double S33 = (cc+ce+ce+ee)*K;
	const double S34 = (cd+cf+de+ef)*K;
	const double S44 = (dd+df+df+ff)*K;

	for (int i=0; i<10; i++) ppJacobian[i]->ZeroMatrix();
	double *x;
	x=ppJacobian[0]->x; x[0]=x[4]=x[8]=S11;
	x=ppJacobian[1]->x; x[0]=x[4]=x[8]=S12;
	x=ppJacobian[2]->x; x[0]=x[4]=x[8]=S13;
	x=ppJacobian[3]->x; x[0]=x[4]=x[8]=S14;
	x=ppJacobian[4]->x; x[0]=x[4]=x[8]=S22;
	x=ppJacobian[5]->x; x[0]=x[4]=x[8]=S23;
	x=ppJacobian[6]->x; x[0]=x[4]=x[8]=S24;
	x=ppJacobian[7]->x; x[0]=x[4]=x[8]=S33;
	x=ppJacobian[8]->x; x[0]=x[4]=x[8]=S34;
	x=ppJacobian[9]->x; x[0]=x[4]=x[8]=S44;
}
	...
	const double& thickness)
{
	...
	m_A11 = jac[0].x[0]; m_A12 = jac[1].x[0]; m_A13 = jac[2].x[0]; m_A14 = jac[3].x[0];
	m_A22 = jac[4].x[0]; m_A23 = jac[5].x[0]; m_A24 = jac[6].x[0];
	m_A33 = jac[7].x[0]; m_A34 = jac[8].x[0];
	m_A44 = jac[9].x[0];
	m_B11 = jac[10].x[0]; m_B12 = jac[11].x[0]; m_B13 = jac[12].x[0]; m_B14 = jac[13].x[0];
	m_B22 = jac[14].x[0]; m_B23 = jac[15].x[0]; m_B24 = jac[16].x[0];
	m_B33 = jac[17].x[0]; m_B34 = jac[18].x[0];
	m_B44 = jac[19].x[0];
	...
	double3x3 *ppJacobian[13])		//jocobian matrices, if needed (not NULL)
{
	...
	const Vector3d u1(xlen-m_len0, 0, 0);
	const Vector3d u0(-u1.x, 0, 0);
	const Vector3d u2 = rot*q2 - m_qq2;
	const Vector3d u3 = rot*q3 - m_qq3;
	const Vector3d u4 = rot*q4 - m_qq4;
	const Vector3d u5 = rot*q5 - m_qq5;
	//force comp.
	FF[0] = (m_A11+m_B11)*u0 + (m_A12+m_B12)*u1 + m_A13*u2 + m_A14*u3 + m_B13*u4 + m_B14*u5;
	FF[1] = (m_A12+m_B12)*u0 + (m_A22+m_B22)*u1 + m_A23*u2 + m_A24*u3 + m_B23*u4 + m_B24*u5;
	FF[2] = m_A13*u0 + m_A23*u1 + m_A33*u2 + m_A34*u3; 
	FF[3] = m_A14*u0 + m_A24*u1 + m_A34*u2 + m_A44*u3; 
	FF[4] = m_B13*u0 + m_B23*u1 + m_B33*u4 + m_B34*u5; 
	FF[5] = m_B14*u0 + m_B24*u1 + m_B34*u5 + m_B44*u5;
	//spring force
	const double E = mtl.getYoung()*m_thickness;
