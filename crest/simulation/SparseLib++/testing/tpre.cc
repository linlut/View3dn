...
int main(int argc, char * argv[])
{
  ...
  const double nm0y = norm(yc-x);
  const double nm0x = norm(x);
  const double nm0 = nm0y/nm0x;
  if (verbose) cout << "ILU yc-x " << nm0 << endl;
  if (nm0 > 1.e-12) {
    errcount++;
    if (verbose) {
      cout << "x" << endl << x << endl;
  ...
  const double nm1 = norm(yr-x)/norm(x);
  if (verbose) cout << "ILU yr-x " << nm1 << endl;
  if (nm1 > 1.e-12) {
    errcount++;
    if (verbose) {
      cout << "x" << endl << x << endl;
