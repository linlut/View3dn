    ...
    if (n == 1)
		...
		for (l = 0; l < n; l++)
			for (i = m - 1; i >= 0; i--) {  
				...
				for (j = pntr[i] + 1; j < pntr[i+1]; j++)
				z += _SpMatVal(c, ldc, indx[j], l) * val[j];
				_SpMatVal(c, ldc, i+pntr[0], l) = (_SpMatVal(b, ldb, i, l) - z) / val[pntr[i]];
    }
  } else {
    if (n == 1)
      for (i = m - 1; i >= 0; i--) {    
   ...
   double *work, const int &lwork)
{
  ...
  if (work && lwork)
	...
	if (transa == 0) {
		if (descra[1] == 1) {
			if (descra[2] == 0)
				CompCol_LowerDiagSolve_double(m, n, unitd, dv, alpha, val, indx, pntr, b, ldb, c, ldc);
			else if (descra[2] == 1)
				CompCol_LowerUnitSolve_double(m, n, unitd, dv, alpha, val, indx, pntr, b, ldb, c, ldc);
			else {
				std::cerr << "Bad descra[2]" << "\n";
				exit(1);
			}
			...
			if (descra[2] == 0)
				CompCol_UpperDiagSolve_double(m, n, unitd, dv, alpha, val, indx, pntr, b, ldb, c, ldc);
			else if (descra[2] == 1)
				CompCol_UpperUnitSolve_double(m, n, unitd, dv, alpha, val, indx, pntr, b, ldb, c, ldc);
			else {
				std::cerr << "Bad descra[2]" << "\n";
				exit(1);
			}
		...
		if (descra[1] == 1) {
			if (descra[2] == 0)
				CompRow_UpperDiagSolve_double(m, n, unitd, dv, alpha, val, indx, pntr,b, ldb, c, ldc);
			else if (descra[2] == 1)
				CompRow_UpperUnitSolve_double(m, n, unitd, dv, alpha, val, indx, pntr,b, ldb, c, ldc);
			else {
				std::cerr << "Bad descra[2]" << "\n";
				exit(1);
			}
			...
			if (descra[2] == 0)
				CompRow_LowerDiagSolve_double(m, n, unitd, dv, alpha, val, indx, pntr,b, ldb, c, ldc);
			else if (descra[2] == 1)
				CompRow_LowerUnitSolve_double(m, n, unitd, dv, alpha, val, indx, pntr,b, ldb, c, ldc);
			else {
				std::cerr << "Bad descra[2]" << "\n";
			exit(1);
		}
