...
COMap::COMap()
{
	...
	m_nCountTCell=0;
	m_dwR=10;//100;//20;//10;//9;//8;//5;// subdivision resolution in mm
...
int COMap::LookUpMap(CrestObjects *pObjects)
{
	...
	CrestObjectInfoV& ObjInfo=pObjects->m_ObjectInfo;
	...
	{
		pV=ObjInfo[i].pVertex;
		pT=ObjInfo[i].pPolygon;
		nLengthV=ObjInfo[i].nVertex;
		nLengthT=ObjInfo[i].nPolygon;
		//if(e_mi.m_iGrab>0) pD->GeoFollowCPos();//if(e_iHold==1 && e_mi.m_iGrab>0) // Important for grasping
		FillOMapT(m_OMap, ObjInfo[i].nID, pV, nLengthV, pT, nLengthT);
	}
	//FillOMapParti(m_OMap);
...
inline void COMap::FillCell(tOMap* m_OMap, long nObject, long nVid, DWORD dwCell)
{
	...
	if (nObject<INSTRUMENT_MAX) m_OMap[Hash(dwCell)][dwCell][nObject].push_back(nVid);
	else{
		tOMap& omap = m_OMap[Hash(dwCell)];
...
{// Vector versioin
			...
			while(pObj0!=obj.end()) // pObj0 to obj
			{
				...
				while(pObj1!=obj.end()) // obj1 goes over
				{
					...
					while(pVector0!=vVector0.end())
					{
						...
						pCurrentV=*pV+(m_tp[0].id[0]=*pCurrentT++)*3;
						v1.x=*(pCurrentV++); v1.y=*(pCurrentV++); v1.z=*pCurrentV;
						pCurrentV=*pV+(m_tp[0].id[1]=*pCurrentT++)*3;
						v2.x=*(pCurrentV++); v2.y=*(pCurrentV++); v2.z=*pCurrentV;
						pCurrentV=*pV+(m_tp[0].id[2]=*pCurrentT)*3;
						...
						while(pVector1!=vVector1.end()) // collision response
						{
							...
							pCurrentV=*(pV+1)+(m_tp[1].id[0]=*pCurrentT++)*3;
							v4.x=*(pCurrentV++); v4.y=*(pCurrentV++); v4.z=*pCurrentV;
							pCurrentV=*(pV+1)+(m_tp[1].id[1]=*pCurrentT++)*3;
							v5.x=*(pCurrentV++); v5.y=*(pCurrentV++); v5.z=*pCurrentV;
							pCurrentV=*(pV+1)+(m_tp[1].id[2]=*pCurrentT)*3;
							v6.x=*(pCurrentV++); v6.y=*(pCurrentV++); v6.z=*pCurrentV;
							//m_nTPairs++;
							//e_performMoniter.MarkTime(&Counter);
							if( !tri_tri_intersection_test_3d((CDFloat*)&v1, (CDFloat*)&v2, (CDFloat*)&v3, 
																(CDFloat*)&v4, (CDFloat*)&v5, (CDFloat*)&v6, 
																&nCoplanar,
																(CDFloat*)&its1, (CDFloat*)&its2) 
																&& 1!=nCoplanar)
							//if( !tri_tri_overlap_test_3d((float*)&v1, (float*)&v2, (float*)&v3, 
							//						(float*)&v4, (float*)&v5, (float*)&v6) )
							//if( !NoDivTriTriIsect((float*)&v1, (float*)&v2, (float*)&v3, 
	...
	CColliResp::ItoTObject(pObjects);
	//// Swap CR buffers. Disable it when timing CD
	//pTissue=e_share.m_pTissueList;
	//while(pTissue)
	//{	
