...
public:
	...
	void setLeftJaw(const int maxnv, vector<Vector3d> pos, vector<int> index);
	void setRightJaw(const int maxnv, vector<Vector3d> pos, vector<int> index);

	//Jaw state: open, close, no change
	int getJawState(void) const
	{
		const double dif = m_jawAngleRadian - m_jawAngleRadianOld;
		if (fabs(dif)<0.1) return 0;
		if (dif>0) 
			return 1;
		else
			return -1;
