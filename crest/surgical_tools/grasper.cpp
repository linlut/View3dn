...
inline void _setJaw(
	...
	Vector3d*& m_pLeftJawVertex, int*& m_pLeftJawVertexIndex, int &m_nLeftJawVertexCount)
{
	int i;
	...
	for (i=0; i<maxnv; i++) 
		if (counters[i]>0) c++;
	assert(c>0);
	m_pLeftJawVertex = new Vector3d[c]; 
	m_pLeftJawVertexIndex = new int [c];
	m_nLeftJawVertexCount = c;
	...
	for (i=0; i<index.size(); i++){
		...
		if (vcount>0){
			m_pLeftJawVertex[c] = pos[i];
			m_pLeftJawVertexIndex[c] = v;
			counters[v]=0;
			c++;
		}
	}
...
void CGrasperHead::setLeftJaw(const int maxnv, vector<Vector3d> pos, vector<int> index)
{
	_setJaw(maxnv, pos, index, m_pLeftJawVertex, m_pLeftJawVertexIndex, m_nLeftJawVertexCount);

}

void CGrasperHead::setRightJaw(const int maxnv, vector<Vector3d> pos, vector<int> index)
{
...
void CGrasperHead::applyRotation(Vector3d *pVertex)
{
    ...
    RotationAxisMatrix(m_axis, m_jawAngleRadian, rot);
    TranslationMatrix(m_translation, trans);
	mat = rot * trans;
