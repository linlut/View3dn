...
#define getAttribute(X) attribute(X)

//using namespace sofa::defaulttype;

void OglModel::parse(ZBaseObjectDescription* arg)
{
	
    //this->core::VisualModel::parse(arg);
    OglModel* obj = this;

    QString s = arg->attribute("normals");
	if (!s.isEmpty())
        obj->setUseNormals(atoi(s)!=0);

    s = arg->attribute("castshadow");
	if (!s.isEmpty())
        obj->setCastShadow(atoi(s)!=0);
/*
...
CIA3D_DECL_CLASS(OglModel)

//int OglModelClass = cia3d::zcore::RegisterObject("Generic visual model for OpenGL display")
//.add< OglModel >();
...
bool OglModel::load(const std::string& filename, const std::string& loader, const std::string& textureName)
{
	/*
	if (textureName != ""){
		QImage *img = QImage::Create(textureName);
		if (img){
			tex = new ogl::Texture(img);
			assert(tex!=NULL);
		}
	}
...
void OglModel::applyRotation(const Quat* q)
{
	/*
	VecCoord& x = *getX();
	for (unsigned int i = 0; i < x.size(); i++)
