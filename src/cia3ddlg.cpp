...
CMaterialTree::CMaterialTree( const QString& appdir, SET_OBJECT_MATERIAL_FUNC_T* pfunc, QWidget *parent, const char *listviewname)
    : QListView( parent, listviewname )
{
	m_pSetObjectMaterial = pfunc;
...
{
	assert(m_pSetObjectMaterial!=NULL);
	QListViewItem * pitem = currentItem();
	if (pitem==NULL) return;
	if (!_isMatitem(pitem))
		return;
