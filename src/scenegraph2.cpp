...
void BestMatchDrawParms(CSceneGraph & sg, CSceneNode *poldnodes, const int nlen, const bool bRecomputeTexture1DRange=true)
{
	...
	for (i=0; i<nsize; i++){
		...
		name1 = p1->GetObjectName();
		
		//first, check name matches;
		for (j=0; j<nlen; j++){
			p2 = &poldnodes[j];
			name2 = poldnodes[j].GetObjectName();
...
CSceneNode* AddOneObjectToSceneGraph(CPolyObj* q1, CSceneGraph &m_SceneGraph, const Vector3f *rgb)
{
	...
	px->SetObjectName(q->GetObjectName());
...
AxisAlignedBox CSceneNode::worldBoundingBox(void) const
{
	...
	AxisAlignedBox wbox;
	wbox.minp = Vector3d(1e30);
	wbox.maxp = Vector3d(-1e30);
	int i;
	for (i=0; i<8; i++){
...
{
	int i;
	const int nobj = s.SceneNodeCount();
	...
	if (deformedVertices.size()<2){ //If there is no deformation input, just return
		...
		Vector3d *baseptr = &deformedVertices[1];  //the fortran array starts from 1
		for (i=0; i<nobj; i++){
...
void CSceneNode::glDraw(const Vector3f &campos, const Vector3f &camdir, const Vector3d *pDeformedVertices, Vector3d *vertexbuff, const bool outputMesh)
{
	...
	if (node.m_pFrame)
		glMultMatrixd(node.m_pFrame->matrix());
...
void CSceneGraph::glDraw(CGLDrawParms &global_drawparm, vector<Vector3d>* pDeformedVertices, const bool outputMesh)
{	
	...
	Vector3d *pointers[120], *vertexbuff=NULL;
	int i, nMaxVBufferLength;
	...
	if (pDeformedVertices){
		...
		for (i=0; i<this->SceneNodeCount(); i++) pointers[i]=NULL;
	}
	...
	for (i=0; i<nsize; i++){
		...
		node.glDraw(campos, camdir, pointers[i], vertexbuff, outputMesh);
		glPopMatrix();
	}

	//render for deformation
...
void CSceneGraph::_glDrawPickedObjectBoundary(void)
{
	...
	const int nsize= SceneNodeCount();
...
void CSceneGraph::_glDrawPickedObjectNormal(void)
{
	...
	case CSceneGraph::PICK_OBJECT:	//draw obj
		_glDrawNodeBoundingBox(m_pActiveSceneNode, Vector3f(1,0,0));
		break;
...
{	
	int c1=0, c2=0;
	objid=-1, faceid=-1;
	for (int i=0; i<scene.SceneNodeCount(); i++){
...
{
	int i, c1, c2;
	for (i=0, c1=0; i<scene.SceneNodeCount(); i++){
		CSceneNode *p = scene.GetSceneNode(i);
		CObject3D *pobj = p->m_pObject;
		if (!pobj) continue;
		c2 = c1+ pobj->GetPickableElementCount(picktype);
...
{
	box.minp = Vector3d(1e30);
	box.maxp = Vector3d(-1e30);
...
{
	int i, j, c=0;
	const int nobj = m.SceneNodeCount();
	for (i=0; i<nobj; i++){
...
{
	int i;
	const int nsize = ps->SceneNodeCount();
...
*/
	...
	for (i=0; i<n; i++){
		CPolyObj *ply = GetSceneNode(i)->m_pObject;
		const int objID = i+1;
	...
	for (i=0, c=1; i<n; i++){
		CSceneNode *p = GetSceneNode(i);
		const double *pmatrix = p->m_pFrame->matrix();
