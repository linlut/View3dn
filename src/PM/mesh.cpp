...
Mesh::Mesh(const float pVertex[][3], const float pTexCoor[][3], const int nv, const int pTri[][3], const int ntri)
{
	int i;
	_numVerts = nv;
	_numTriangles = ntri;
	...
	for (i=0; i<_numVerts; i++){
		const float *pv = pVertex[i];
		const float *pt = pTexCoor[i];
		vertex v(pv[0], pv[1], pv[2], pt[0], pt[1], pt[2]);
		v.setIndex(i);
		_vlist.push_back(v); 
	}
