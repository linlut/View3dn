...
static inline void COPY_VERTEX_TEXCOOR(float pVertex[][3], float pTexCoor[][3], const int v0, const float pos[3], const float tex[3])
{
	const float *psrc;
	float *pdst;
	psrc = pos, pdst = pVertex[v0];
	pdst[0]=psrc[0], pdst[1]=psrc[1], pdst[2]=psrc[2];
	psrc = tex, pdst = pTexCoor[v0];
	pdst[0]=psrc[0], pdst[1]=psrc[1], pdst[2]=psrc[2];
}

void MeshSimplification(float pVertex[][3], float pTexCoor[][3], const int nv, int pTri[][3], const int ntri, const float simpratio, int &newnv, int &newntri)
{
	int i;
	triangle t;
	newnv = newntri = 0;
	PMesh::EdgeCost edgemethod = PMesh::QUADRIC;

	////construct a mesh class instance
	Mesh mesh(pVertex, pTexCoor, nv, pTri, ntri);
	PMesh progmesh(&mesh, edgemethod);

	///collapse the edges
	int ncollapse = (int)(nv*(1-simpratio));
	bool ret = true;
	for (i=0; ret && i<ncollapse; i++)
		ret = progmesh.collapseEdge();

	///collect the result and renumber the vertices;
	int *vindex = new int[nv];
	assert(vindex!=NULL);
	for (i=0; i<nv; i++) vindex[i]=-1;

	for (i=0; i<ntri; i++){
		if (progmesh.getTri(i, t) && t.isActive()){
			const int v1 = t.getVert1Index();
			const int v2 = t.getVert2Index();
			const int v3 = t.getVert3Index();
			if (vindex[v1]<0) vindex[v1]=newnv++;
			if (vindex[v2]<0) vindex[v2]=newnv++;
			if (vindex[v3]<0) vindex[v3]=newnv++;
			const int n1 = vindex[v1];
			const int n2 = vindex[v2];
			const int n3 = vindex[v3];
			///save the new triangle
			int *pt = pTri[newntri++];
			pt[0] = n1, pt[1] = n2, pt[2] = n3;
			///save the new vertices
			COPY_VERTEX_TEXCOOR(pVertex, pTexCoor, n1, t.getVert1(), t.getVert1TexCoor());
			COPY_VERTEX_TEXCOOR(pVertex, pTexCoor, n2, t.getVert2(), t.getVert2TexCoor());
			COPY_VERTEX_TEXCOOR(pVertex, pTexCoor, n3, t.getVert3(), t.getVert3TexCoor());
		}
	}
