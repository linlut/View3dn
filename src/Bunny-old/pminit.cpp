...
float DeltaT = 0.1f;	// change in time since last frame
...
List<Vector> vert(30720);		// global list of vertices
List<tridata> tri(30720);		// global list of triangles
List<int> collapse_map(30720);	// to which neighbor each vertex collapses
...
void GetRabbitData(const float pVertex[][3], const int nv, const int pTriangle[][3], const int ntri)
{
	...
	int i;
	for(i=0;i<nv;i++) {
		const float *vp=pVertex[i];
		vert.Add(Vector(vp[0],vp[1],vp[2]));
	}
	for(i=0;i<ntri;i++) {
		tridata td;
		td.v[0]=pTriangle[i][0];
		td.v[1]=pTriangle[i][1];
		td.v[2]=pTriangle[i][2];
		tri.Add(td);
	}
...
void InitModel(const float pVertex[][3], const int nv, const int pTriangle[][3], const int ntri)
{
	List<int> permutation;
	GetRabbitData(pVertex, nv, pTriangle, ntri);  
...
void PMSimplification(const float pVertex[][3], const int nv, const int pTriangle[][3], const int ntri)
{
	InitModel(pVertex, nv, pTriangle, ntri);
