...
{
 public: ImageInterface(QWidget *parent) : QDialog(parent) 
	{ 
		setupUi(this);
		imgWidth->setValue(parent->width());
		imgHeight->setValue(parent->height());
	}
};
#endif
...
bool QGLViewer::saveImageSnapshot(const QString& fileName)
{
  ...
  // 1 means never set : use current window size as default
	if (1) //(imageInterface->imgWidth->value() == 1) && (imageInterface->imgHeight->value() == 1))
    {
