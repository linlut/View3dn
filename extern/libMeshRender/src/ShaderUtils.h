...
{
	if (originMacro == NULL || newMacro == NULL) return;

	const int n0 = strlen(originMacro);
	const int n1 = strlen(newMacro);
	if (n1 != 0 && n0 == n1){
		char *p = text;
        ...
        fprintf(stderr, "Error in macro processing: length not equal!\n");
    }
}
...
// Handles macro replacement, but not #include
...
{
    //must read file as binary to prevent problems of newline translation
